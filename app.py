...
def rank_candidates(df_pending: pd.DataFrame, schema: dict, model, top_k: int = 10) -> pd.DataFrame:
    ...
    df_aligned["score"] = pd.Series(scores, index=df_aligned.index).round(2)  # ✅ corrigido
...
if uploaded is not None:
    ...
    # 🔹 Filtro de vagas com ID + nome (se existir coluna 'vaga_nome')
    if "vaga_nome" in ranking.columns:
        ranking["vaga_display"] = ranking[schema["id_vaga_col"]].astype(str) + " - " + ranking["vaga_nome"].astype(str)
    else:
    ...
    for i, (_, row) in enumerate(top.iterrows()):
        with cols[i % 3]:
            ...
            empresa = row.get("empresa", "Empresa não informada")
            st.markdown(f"### 👤 {nome}")
    ...
    with st.expander("📊 Tabela completa da vaga"):
        cols_show = [schema["id_cand_col"], "nome_candidato", "empresa", "score", "rank"]
